...
expName = 'practica_temps_lectura'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Brainvitge\\Desktop\\Clara\\pilot_eeg_Mundet\\training_temps_lectura_lastrun.py',
...
start_txt = visual.TextStim(win=win, name='start_txt',
    text="INSTRUCCIONES\n\nAhora vas a oír unas frases. Escúchalas con atención. Después de cada frase aparecerá la pregunta '¿Era correcta la frase anterior?' \n\nPulsa la tecla Q para indicar que SÍ y la tecla P para indicar que NO.\n\nEs importante que mientras escuchas la frase no parpadees y fijes tu mirada en la cruz que aparecerá en el centro de la pantalla.\n\nVamos a emplezar con una práctica.\n\nPulsa la barra espaciadora para empezar.",
    font='Arial',
...
end_trainingClock = core.Clock()
end_t = visual.TextStim(win=win, name='end_t',
...
load = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('Trigers_Práctica.xlsx'),
    seed=None, name='load')
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if port_onset_stim.status == NOT_STARTED and t >= fixDur-frameTolerance:
            # keep track of start time/frame for later
        ...
        if port_fix.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > port_fix.tStartRefresh + 0.01-frameTolerance:
                # keep track of stop time/frame for later
...
# update component parameters for each repeat
# keep track of which components have finished
end_trainingComponents = [end_t]
for thisComponent in end_trainingComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
end_trainingClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    tThisFlip = win.getFutureFlipTime(clock=end_trainingClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *end_t* updates
    if end_t.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        end_t.frameNStart = frameN  # exact frame index
        end_t.tStart = t  # local t and not account for scr refresh
        end_t.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(end_t, 'tStartRefresh')  # time at next scr refresh
        end_t.setAutoDraw(True)
    if end_t.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > end_t.tStartRefresh + 2.0-frameTolerance:
            # keep track of stop time/frame for later
            end_t.tStop = t  # not accounting for scr refresh
            end_t.frameNStop = frameN  # exact frame index
            win.timeOnFlip(end_t, 'tStopRefresh')  # time at next scr refresh
...
for thisComponent in end_trainingComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('end_t.started', end_t.tStartRefresh)
thisExp.addData('end_t.stopped', end_t.tStopRefresh)
